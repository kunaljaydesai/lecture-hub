...
public class Database {
    ...
    public void createTables() {
        try {
            ...
            statement.addBatch("CREATE TABLE MESSAGES (ID INTEGER NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1)," +
                    "AUTHOR_NAME VARCHAR(155), ROOM_NUM INTEGER, MESSAGE VARCHAR(155), SLIDE_NUMBER INTEGER)");
    ...
    public static void addMessage(Message m) {
        String sqlStatement = "INSERT INTO MESSAGES " +
                "(AUTHOR_NAME, ROOM_NUM, MESSAGE, SLIDE_NUMBER) VALUES ('" +
                m.getAuthor() + "', " + m.getRoomName() + ", '" + m.getMessage() + ", '" + m.getSlideNumber() + "')";
        try {
            Statement statement = connection.createStatement();
    ...
    public static Room getRoom(Room r) {
        String sqlStatement = "SELECT * FROM ROOMS WHERE ROOM_NAME='" +  r.getName() + "'";
        try {
            Statement statement = connection.createStatement();
